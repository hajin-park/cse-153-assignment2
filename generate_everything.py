...
def generate_random_baselines(output_dir, duration_seconds=7.4):
    ...
    for i in range(3):
        ...
        random_patterns.append(
            {"file": f"random_drums_{i+1}.mid", "events": len(drums.notes)}
        )
...
"""
    ...
    model_report = {
        ...
        "training_results": {
            "correlation_with_movement": analysis["generated_analysis"][
                "movement_correlation"
            ],
            "vs_original_correlation": analysis["original_analysis"][
                "movement_correlation"
            ],
            "quality_assessment": analysis["alignment_quality"][
...
# ------------------------------------------------------------------
def _run(cmd):
    """Return True if subprocess finishes with exit‑code 0."""
    try:
        subprocess.run(
...
def _midi_to_wav(midi_path: Path, wav_path: Path) -> bool:
    """Convert MIDI to WAV using whatever demuxer FFmpeg supports."""
    # 2) try demuxer 'midi' (some builds use this alias)
    if _run(["ffmpeg", "-y", "-f", "midi", "-i", str(midi_path), str(wav_path)]):
        return True
    return False
...
def make_synced_video(gif_path, midi_path, out_path):
    """
    Mux dance GIF and drum MIDI (if convertible) into MP4.
    Falls back to video‑only if MIDI synthesis fails.
    """
    ...
    tmp_mp4 = gif_path.with_name(gif_path.stem + "_tmp.mp4")

    # 1) GIF → MP4 (video track)
    _run(
        [
            "ffmpeg",
            "-y",
    ...
    # 2) MIDI → WAV  (only if conversion succeeds)
    have_audio = midi_path.exists() and _midi_to_wav(midi_path, wav_path)

    # 3) Mux (or just rename if no audio)
    if have_audio:
        _run(
            [
                "ffmpeg",
        ...
        print(
            f"[!] Could not convert {midi_path.name} to audio" f"video will be silent."
        )
...
def generate_everything_for_version(version=7, sample_idx=58):
    ...
    print(
        f"Processing dance sample {sample_idx}: {sample_info.get('original_file', 'unknown')}"
    )
    ...
    try:
        ...
        video_path = create_pose_visualization(
            raw_pose_data, output_dir / "dance_video_FIXED.gif"
        )
    except Exception as e:
        print(f"Video creation failed: {e}")
    ...
    random_patterns = generate_random_baselines(
        output_dir, duration_seconds=len(pose_data) / 60.0
    )

    out_dir = Path(output_dir)
    dance_gif = out_dir / "dance_video_FIXED.gif"
    make_synced_video(
        dance_gif, out_dir / "original_drums.mid", out_dir / "orig_sync.mp4"
    )
    make_synced_video(
        dance_gif, out_dir / "generated_drums.mid", out_dir / "gen_sync.mp4"
    )
    print("Synced MP4s written (orig_sync.mp4 / gen_sync.mp4)")
...
def create_output_directory(version):
    """Create and return the output directory path."""
    base_dir = f"version_{version}/complete_output"
    os.makedirs(base_dir, exist_ok=True)
    return base_dir


def generate_all_outputs(checkpoint_path, version):
    """Generate all outputs for analysis."""
    output_dir = create_output_directory(version)

    # 1. Generate drums from the model
    print("Generating drums from model...")
    cmd = f"python generate_drums.py --checkpoint {checkpoint_path} --output_dir {output_dir}"
    stdout, stderr, rc = run_command(cmd)
    if rc != 0:
        print(f"Error generating drums: {stderr}")
        return False

    # 2. Create dance visualization
    print("Creating dance visualization...")
    try:
        # Load pose data from sample 58 (the reference sample)
        pose_path = "dataset_root/sample_058/pose.npy"
        if os.path.exists(pose_path):
            pose_data = np.load(pose_path)
            video_path = create_pose_visualization(
                pose_data, Path(output_dir) / "dance_video_FIXED.gif"
            )
            print(f"Dance video created: {video_path}")
        else:
            print("Warning: Could not find pose data for visualization")
    except Exception as e:
        print(f"Warning: Dance video creation failed: {e}")

    # 3. Create random baselines
    print("Creating random baselines...")
    cmd = f"python create_random_baseline.py"
    stdout, stderr, rc = run_command(cmd)
    if rc != 0:
        print(f"Error creating random baselines: {stderr}")
        return False

    # 4. Run alignment analysis
    print("Running alignment analysis...")
    cmd = f"python alignment_analysis.py --midi_file {os.path.join(output_dir, 'generated_drums.mid')} --pose_file dataset_root/sample_058/pose.npy --output_dir {output_dir}"
    stdout, stderr, rc = run_command(cmd)
    if rc != 0:
        print(f"Error running alignment analysis: {stderr}")
        return False

    # 5. Copy original files for reference
    shutil.copy(
        "dataset_root/sample_058/drums.mid",
        os.path.join(output_dir, "original_drums.mid"),
    )
    if os.path.exists("dataset_root/sample_058/pose.npy"):
        shutil.copy(
            "dataset_root/sample_058/pose.npy", os.path.join(output_dir, "pose.npy")
        )
    ...
    report = {
        ...
        "outputs_generated": {
            ...
            "visualization": "dance_video_FIXED.gif",
            "analysis": "alignment_analysis.json",
            "plots": "correlation_plot.png",
        },
    }

    with open(os.path.join(output_dir, "generation_report.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--version", type=str, required=True, help="Version number for output directory"
    )
    args = parser.parse_args()

    success = generate_all_outputs(args.checkpoint, args.version)
    if not success:
        print("Failed to generate all outputs")
